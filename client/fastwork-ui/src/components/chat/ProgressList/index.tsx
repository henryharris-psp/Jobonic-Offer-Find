...
const ProgressList: React.FC = () => {
    ...
    // Fetch milestone data from backend when component mounts
    useEffect(() => {
        const fetchMilestones = async () => {
            try {
                const response = await httpClient.get('/checkpoint/serviceId?serviceId'); // Replace with your API endpoint
    ...
    const progressMilestonePayload = {
                    ...
                    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
                        <div className="flex flex-col w-full max-w-lg p-8 rounded-xl bg-[#CFEDF4]">
                            ...
                            <div className="flex-1 mt-4">
                                <p className="font-bold text-md ml-1 mb-2">Description</p>
                                <input
                                    type="text"
                                    name="todoDescription"
                                    placeholder="To-Do Description"
                                    value={milestone.todoDescription}
                                    onChange={handleInputChange}
                                    className="border-green-200 rounded-xl bg-[#e8f3f3] focus:outline-0 hover:outline-0 w-full"
                                />
                            </div>
                        ...
                        >
                            <span
                                ...
                                <button onClick={() => removeMilestone(milestone.id)}
                                    ...
                                    <TrashIcon className="size-4 ml-4 text-gray-600 cursor-pointer"/>
                                </button>
                            </div>
