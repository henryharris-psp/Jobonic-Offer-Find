...
import { BanknotesIcon, CheckIcon, CloudArrowUpIcon, DocumentPlusIcon, HandThumbUpIcon, XMarkIcon } from "@heroicons/react/24/solid";
...
const MilestoneProgressSection = ({
    ...
    const [isOpen, setIsOpen] = useState<boolean>(isCurrent);
    const { 
        latestContract, 
        authUserType, 
        updateChatRoom, 
    ...
    //methods
        ...
        //freelancer actions
            ...
            const handleOnClickDone = async () => {
                if(confirm('Are you sure to mark this milestone as done without uploading any file?')){
                    submit();
                }
            }
        ...
        //file upload handlers
            ...
            const handleOnDeleteFile = (fileId: string, status: FileStatus) => {
            ...
            if(isUploading && targetFile){
                ...
                ( async () => {
                    try {
                        ...
                        const { id, name, fileSize } = res.data;
                                ...
                                <div className={`border rounded-lg ${
                                    ...
                                    <div className="flex flex-col space-y-3 px-2 pt-2 pb-3">
                                        ...
                                        { allFiles.map( (file) => 
                                            <FileItem
                                                key={file.id}
                                                isDeletable={authUserType === 'freelancer' && status === 'waiting_for_submission'}
